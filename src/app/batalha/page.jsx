...
export default function BattlePage() {
  ...
  const awardVictory = useCallback(async (matchId) => {
    const reward = await webStore.rewardVictory(matchId, COINS_PER_WIN);
    dispatch(actCoins(reward.coins));
    return reward;
  }, [dispatch]);
  const rewardFinishedBattle = useCallback((previous, next, localRole) => {
    if (previous?.status !== "finished" && next?.status === "finished" && next.winner === localRole) void awardVictory(next.matchId);
    return next;
  }, [awardVictory]);
    ...
    try {
      ...
      realtime.current = createBattleRoom(code, currentPlayer, {
        ...
        onEvent: ({ type, payload }) => {
          ...
          if (type === BATTLE_EVENTS.START || type === BATTLE_EVENTS.STATE) { setBattle((previous) => rewardFinishedBattle(previous, payload, currentRole)); setScreen("battle"); setNotice("BATALHA INICIADA!"); }
          if (type === BATTLE_EVENTS.ACTION && currentRole === "host") setBattle((previous) => { if (!previous) return previous; const next = resolveAction(previous, "guest", payload); broadcast(BATTLE_EVENTS.STATE, next); return rewardFinishedBattle(previous, next, currentRole); });
  ...
  useEffect(() => {
    ...
    cpuTimer.current = setTimeout(() => setBattle((current) => {
      ...
      return rewardFinishedBattle(current, resolveAction(current, "guest", shouldHeal ? { type: "potion", targetPokemonId: active.id } : { type: "attack", moveId: useSpecial ? "type-strike" : "strike" }), "host");
    }), 850);
