...
export default function Shop() {
  ...
  useEffect(() => { let active = true; getShopCatalog().then((data) => { if (active) setCatalog(data); }).catch(() => { if (active) setCatalogError(true); }); return () => { active = false; }; }, []);
  ...
  const total = searching ? items.length : filteredCatalog.length; const pages = Math.max(1, Math.ceil(total / PAGE_SIZE));
  const pageCatalog = useMemo(() => filteredCatalog.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE), [filteredCatalog, page]);
  useEffect(() => { if (page > pages) setPage(pages); }, [page, pages]);
  useEffect(() => { let active = true; const request = async () => { setLoading(true); try { if (query.trim()) { const pokemon = await loadPokemon(query).catch(() => null); if (active) setItems(pokemon ? [pokemon] : []); return; } if (!catalog.length) { if (active) setItems([]); return; } const pokemon = await Promise.all(pageCatalog.map(async (item) => { const detail = await loadPokemon(item.name).catch(() => null); return detail ? { ...detail, rarity: item.rarity } : null; })); if (active) setItems(pokemon.filter(Boolean)); } catch { if (active) setItems([]); } finally { if (active) setLoading(false); } }; const timer = setTimeout(request, query.trim() ? 350 : 0); return () => { active = false; clearTimeout(timer); }; }, [pageCatalog, query, catalog.length]);
