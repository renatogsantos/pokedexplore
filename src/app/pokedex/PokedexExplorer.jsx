...
function Ball({ size = 24 }) {
  return (
    <img
      className="collection-ball-icon"
      src="/pokeball.png"
      width={size}
      height={size}
      alt=""
...
function CollectionCard({ pokemon, onOpen, index }) {
  ...
  const color =
    pokemonData.find((item) => item.type === primary)?.color || "#64748b";
  return (
    <motion.button
      type="button"
      className={`collection-card ${getRarityClassName(pokemon)}`}
    ...
    >
      ...
      <span className="collection-types">
        {types.map((type) => (
...
export default function PokedexExplorer() {
  ...
  const { Pokedex, Pokemon, OpenCardPokedex, OpenCardPokemon } = useSelector(
  ...
  useEffect(() => {
    webStore
      .getData("Pokedex")
      .then((collection) => dispatch(actAddPokedex(collection)));
  }, [dispatch]);

  const collection = Pokedex || [];
  const totalLevels = collection.reduce(
    (sum, pokemon) => sum + getPokemonLevel(pokemon),
    0,
  );
    ...
    () =>
      collection
        .filter((pokemon) => {
          ...
          const types =
            pokemon.types?.map((item) => item.type?.name || item.name) || [];
          return (
            matchesName &&
  ...
  return (
    <main className="collection-page">
      <a className="collection-skip" href="#collection-content">
        Ir para sua coleção
      </a>
      ...
      <div className="collection-shell" id="collection-content">
        <section className="collection-hero">
          <div>
            <span className="collection-eyebrow">SUA COLEÇÃO</span>
            <h1>Pokédex</h1>
            <p>
              Capture novos parceiros, fortaleça os repetidos e monte sua melhor
              equipe.
            </p>
        ...
        <section className="collection-stats" aria-label="Resumo da coleção">
          <div>
            <Ball size={24} />
          ...
          <div>
            ...
            <span>níveis somados</span>
          </div>
          <div>
            <GameController weight="fill" aria-hidden="true" />
            <strong>{Math.min(collection.length, 3)} / 3</strong>
            <span>prontos para lutar</span>
          </div>
          <div className="collection-progress">
            <span>Progresso da coleção</span>
            <strong>{collection.length} de 1000</strong>
            <i>
              <b
                style={{ width: `${Math.min(collection.length / 10, 100)}%` }}
              />
            </i>
          </div>
        </section>
        ...
        >
          <div className="collection-browser-heading">
            <div>
              <span className="collection-eyebrow">SEUS POKÉMON</span>
            ...
            <button
              type="button"
              className="collection-capture mini"
              onClick={() => dispatch(getPokemonToPokedex())}
            >
              <Ball size={19} /> Capturar
            </button>
          </div>
          <div className="collection-controls">
            ...
            <div
              className="collection-type-filters"
              ...
              <div className="collection-grid">
                {visiblePokemon.map((pokemon, index) => (
                  <CollectionCard
                    ...
                    pokemon={pokemon}
                    index={index}
                    onOpen={(name) => dispatch(getPokemon(name))}
                  />
                ))}
              </div>
              <PokemonPagination
            ...
            <div className="collection-empty">
              ...
              <h3>
                {collection.length
                  ...
                  : "Sua Pokédex está esperando"}
              </h3>
              <p>
                {collection.length
                  ? "Tente outro nome ou tipo."
                  : "Abra uma Pokébola para fazer sua primeira captura."}
              </p>
              <button
                type="button"
                className="collection-capture"
                onClick={() => dispatch(getPokemonToPokedex())}
              >
                <Ball size={21} /> Capturar Pokémon
              </button>
            </div>
          )}
        </section>
        <section className="collection-tip">
          <Sparkle size={28} weight="fill" aria-hidden="true" />
          <div>
            <strong>Encontrar um repetido é uma boa notícia.</strong>
            <span>
              Cada captura repetida aumenta 1 nível, até o nível{" "}
              {MAX_POKEMON_LEVEL}.
            </span>
          </div>
      ...
      <AnimatePresence>
        {OpenCardPokedex && Pokemon && (
          <div className="card-add-pokemon-box" onClick={closeCapture}>
            <CardAddPokemon pokemon={Pokemon} />
          </div>
        )}
      </AnimatePresence>
      <AnimatePresence>
        {OpenCardPokemon && Pokemon && (
          <div className="card-pokemon-box" onClick={closeDetail}>
