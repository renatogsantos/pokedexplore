...
import { ITEM_CATALOG, getItemUsagePresentation, getRarityLabel } from "@/lib/items/catalog";
...
export default function InventoryPage() {
  ...
  const items = ITEM_CATALOG.filter((item) => (economy.inventory?.[item.id] || 0) > 0).filter((item) => filter === "all" || (filter === "held" && item.usageType === "HELD") || (filter === "bag" && item.usageType === "BAG") || (filter === "consumable" && item.consumable) || (filter === "artifact" && item.category === "ARTIFACT"));
