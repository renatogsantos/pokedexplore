...
import { isSupportedStatus, normalizeStatusEffect } from "@/lib/battle/statuses";
...
export const PARALYSIS_ACTION_BLOCK_CHANCE = .25;
...
const TYPE_MOVES = Object.freeze({
  ...
  fire: [move("Ember", { damageClass: "special", statusEffect: { id: "burn", chance: .1 } }), move("Fire Fang", { statusEffect: { id: "burn", chance: .1 } }), move("Flamethrower", { damageClass: "special", statusEffect: { id: "burn", chance: .1 } })],
  water: [move("Water Gun", { damageClass: "special" }), move("Aqua Tail"), move("Hydro Pump", { damageClass: "special" })],
  grass: [move("Vine Whip"), move("Razor Leaf"), move("Solar Beam", { damageClass: "special" })],
  electric: [move("Thunder Shock", { damageClass: "special", statusEffect: { id: "paralysis", chance: .1 } }), move("Spark", { statusEffect: { id: "paralysis", chance: .3 } }), move("Thunderbolt", { damageClass: "special", statusEffect: { id: "paralysis", chance: .1 } })],
  ice: [move("Powder Snow", { damageClass: "special" }), move("Ice Fang"), move("Ice Beam", { damageClass: "special" })],
  fighting: [move("Karate Chop"), move("Brick Break"), move("Close Combat")],
  poison: [move("Poison Sting", { statusEffect: { id: "poison", chance: .3 } }), move("Acid", { damageClass: "special" }), move("Sludge Bomb", { damageClass: "special", statusEffect: { id: "poison", chance: .3 } })],
  ground: [move("Mud-Slap", { damageClass: "special" }), move("Bulldoze"), move("Earthquake")],
  flying: [move("Peck"), move("Wing Attack"), move("Air Slash", { damageClass: "special" })],
  psychic: [move("Confusion", { damageClass: "special" }), move("Psybeam", { damageClass: "special" }), move("Psychic", { damageClass: "special" })],
  bug: [move("Bug Bite"), move("X-Scissor"), move("Bug Buzz", { damageClass: "special" })],
  rock: [move("Rock Throw"), move("Rock Slide"), move("Stone Edge")],
  ghost: [move("Astonish"), move("Shadow Sneak"), move("Shadow Ball", { damageClass: "special" })],
  dragon: [move("Dragon Breath", { damageClass: "special" }), move("Dragon Claw"), move("Dragon Pulse", { damageClass: "special" })],
  dark: [move("Bite"), move("Assurance"), move("Dark Pulse", { damageClass: "special" })],
  steel: [move("Metal Claw"), move("Iron Head"), move("Flash Cannon", { damageClass: "special" })],
  fairy: [move("Fairy Wind", { damageClass: "special" }), move("Draining Kiss", { damageClass: "special" }), move("Moonblast", { damageClass: "special" })],
});
...
const addStatusEvent = (effect, event) => { if (!event) return; effect.statusEvents = [...(effect.statusEvents || []), event]; effect.statusEvent = event; };
...
export function getBattleMoves(pokemon) {
  ...
  const slots = [
    { id: `${type}-quick`, type, power: 40, accuracy: 100, damageClass: "physical", special: false },
    { id: `${type}-steady`, type, power: 60, accuracy: 100, damageClass: "physical", special: false },
    { id: `${type}-special`, type, power: 90, accuracy: 100, damageClass: "special", special: true },
  ];
  return definitions.map((definition, index) => normalizeBattleMove(definition, slots[index]));
}
...
function prepareFighter(pokemon) {
  ...
  const defaults = getBattleMoves({ ...pokemon, types }); const custom = Array.isArray(pokemon.moveset) ? pokemon.moveset.filter((entry) => entry?.id && Number.isFinite(Number(entry?.power))).map((entry) => normalizeBattleMove(entry)) : [];
...
function applySupportedStatus(target, statusId, context, effect, eventId) {
  if (!target || target.hp <= 0 || target.status || !isSupportedStatus(statusId)) return null;
  const statusEvent = {
    type: "STATUS_APPLIED",
    status: statusId,
    successful: true,
  ...
  if (["arcane-mirror", "celestial-clock"].includes(prevention)) {
    ...
    addItemEvent(effect, { ...consumeHeld(target, prevention, eventId, { type: "prevent_status", status: statusId, healing: recovery }), owner: context.targetRole });
    addStatusEvent(effect, { ...statusEvent, type: "STATUS_PREVENTED", successful: false, itemId: prevention });
    return null;
  }
  target.status = {
    id: statusId,
  ...
  if (activeItem(target, "purifier")) {
    target.status = null;
    addItemEvent(effect, { ...consumeHeld(target, "purifier", eventId, { type: "cure_status", status: statusId }), owner: context.targetRole });
    addStatusEvent(effect, { ...statusEvent, type: "STATUS_CURED", sourceKind: "item", itemId: "purifier" });
  }
  return statusId;
}
...
function finishActorTurn(next, actor, effect) {
  ...
  if (fighter.status && ["burn", "poison"].includes(fighter.status.id)) { const damage = Math.max(1, Math.ceil(fighter.maxHp * STATUS_DAMAGE_PERCENTAGE)); fighter.hp = Math.max(0, fighter.hp - damage); effect.endStatus = { status: fighter.status.id, damage }; addStatusEvent(effect, { type: "STATUS_DAMAGE", status: fighter.status.id, damage, targetRole: actor, targetPokemonId: fighter.id, targetPokemonName: fighter.name, sourcePokemonId: fighter.status.sourcePokemonId || null, sourcePokemonName: fighter.status.sourcePokemonName || null, moveId: fighter.status.sourceMoveId || null, moveName: fighter.status.sourceMoveName || null }); }
...
function resolveBagAction(state, next, actor, enemy, action) {
  ...
  next[actor].bag[itemId] = quantity - 1; next[actor].potionsRemaining = next[actor].bag["vital-potion"] || 0; next.turn = enemy; next.log = `${definition.name} usado em ${target.name}!`; next.effect = { kind: "item", itemId, itemName: definition.name, actor, target: actor, targetPokemonId: target.id, targetPokemonName: target.name, targetIndex, healing, curedStatus, remaining: next[actor].bag[itemId], statusEvents: curedStatus ? [{ type: "STATUS_CURED", status: curedStatus, successful: true, sourceKind: "item", itemId, targetRole: actor, targetPokemonId: target.id, targetPokemonName: target.name }] : [], result: { applied: true, consumed: true, itemId, targetPokemonId: target.id, healing, curedStatus } }; next.revision += 1; return next;
}
...
export function resolveAction(state, actor, action) {
  ...
  if (fighter.status?.id === "sleep") { const sleepingStatus = { ...fighter.status }; fighter.status.turns -= 1; const statusEvents = [{ type: "STATUS_TRIGGERED", status: "sleep", successful: true, preventedAction: true, targetRole: actor, targetPokemonId: fighter.id, targetPokemonName: fighter.name, sourcePokemonId: sleepingStatus.sourcePokemonId || null, sourcePokemonName: sleepingStatus.sourcePokemonName || null, moveId: sleepingStatus.sourceMoveId || null, moveName: sleepingStatus.sourceMoveName || null }]; if (fighter.status.turns <= 0) { fighter.status = null; statusEvents.push({ type: "STATUS_EXPIRED", status: "sleep", successful: true, targetRole: actor, targetPokemonId: fighter.id, targetPokemonName: fighter.name }); } next.turn = enemy; next.log = `${fighter.name} continua dormindo e não pode atacar neste turno.`; next.effect = { kind: "status", actor, target: actor, status: "sleep", statusEvents, statusEvent: statusEvents[statusEvents.length - 1] }; next.revision += 1; return next; }
  if (fighter.status?.id === "paralysis" && nextRandom(next) < PARALYSIS_ACTION_BLOCK_CHANCE) { next.turn = enemy; next.log = `${fighter.name} está paralisado e não conseguiu agir!`; const statusEvent = { type: "STATUS_TRIGGERED", status: "paralysis", successful: true, preventedAction: true, targetRole: actor, targetPokemonId: fighter.id, targetPokemonName: fighter.name, sourcePokemonId: fighter.status.sourcePokemonId || null, sourcePokemonName: fighter.status.sourcePokemonName || null, moveId: fighter.status.sourceMoveId || null, moveName: fighter.status.sourceMoveName || null }; next.effect = { kind: "status", actor, target: actor, status: "paralysis", statusEvents: [statusEvent], statusEvent }; next.revision += 1; return next; }
  if (nextRandom(next) > (move.accuracy ?? 100) / 100) { next.turn = enemy; next.log = `${fighter.name} errou ${move.name}!`; next.effect = { kind: "miss", actor, target: enemy, type: move.type, moveId: move.id, moveName: move.name, sourcePokemonId: fighter.id, sourcePokemonName: fighter.name, targetPokemonId: defender.id, targetPokemonName: defender.name }; next.revision += 1; return next; }
  const attackType = move.type === "own" ? fighter.type : move.type; const resolution = calculateDamage({ attacker: fighter, defender, move, variance: 1 - DAMAGE_BALANCE.VARIANCE + nextRandom(next) * DAMAGE_BALANCE.VARIANCE * 2 }); const eventId = action.actionId || `${actor}:${state.revision + 1}:${fighter.id}:${defender.id}:${move.id}`; const beforeHp = defender.hp; let damage = Math.min(beforeHp, resolution.damage); const effect = { kind: "attack", actor, target: enemy, type: attackType, damage, effective: resolution.effectiveness > 1, special: move.special, moveId: move.id, moveName: move.name, sourcePokemonId: fighter.id, sourcePokemonName: fighter.name, targetPokemonId: defender.id, targetPokemonName: defender.name, damageResolution: resolution, itemEvents: [], statusEvents: [] };
  ...
  let status = null; let reactiveAbility = null;
  if (!defender.status && defender.hp > 0 && move.statusEffect) {
    const successful = nextRandom(next) < move.statusEffect.chance;
    if (successful) status = applySupportedStatus(defender, move.statusEffect.id, { sourcePokemon: fighter, sourceRole: actor, targetRole: enemy, sourceKind: "move", move, chance: move.statusEffect.chance, appliedTurn: state.revision + 1 }, effect, eventId);
    else addStatusEvent(effect, { type: "STATUS_ATTEMPTED", status: move.statusEffect.id, successful: false, sourceKind: "move", sourcePokemonId: fighter.id, sourcePokemonName: fighter.name, sourceRole: actor, targetPokemonId: defender.id, targetPokemonName: defender.name, targetRole: enemy, moveId: move.id, moveName: move.name, chance: move.statusEffect.chance, eventId });
  }
  if (defender.ability === "static" && move.damageClass === "physical" && nextRandom(next) < .2) { const applied = applySupportedStatus(fighter, "paralysis", { sourcePokemon: defender, sourceRole: enemy, targetRole: actor, sourceKind: "ability", abilityId: "static", chance: .2, appliedTurn: state.revision + 1 }, effect, eventId); status = applied || status; if (applied) reactiveAbility = "static"; }
  if (damage > 0 && activeItem(defender, "poison-thorn") && nextRandom(next) < .2) { const poisoned = applySupportedStatus(fighter, "poison", { sourcePokemon: defender, sourceRole: enemy, targetRole: actor, sourceKind: "item", itemId: "poison-thorn", chance: .2, appliedTurn: state.revision + 1 }, effect, eventId); if (poisoned) { status = poisoned; addItemEvent(effect, { itemId: "poison-thorn", pokemonId: defender.id, targetPokemonId: fighter.id, consumed: false, eventId, effect: { type: "status", status: "poison" } }); } }
