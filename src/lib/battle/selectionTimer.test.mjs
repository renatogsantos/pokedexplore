...
import { completeSelection, createSelectionTiming, getReadySelection, getSelectionTimerState } from "./selectionTimer.js";
...
test("manual ready ignores the click event and keeps the selected team", () => {
  const selected = [{ id: "a" }, { id: "b" }, { id: "c" }];
  assert.equal(getReadySelection({ type: "click" }, selected), selected);
  assert.equal(getReadySelection(selected, []), selected);
});
