...
export function calculateBattleRewards({ won, durationMs, usedOnlyOnePokemon, championBonusEligible = false, baseCoins = BASE_VICTORY_COINS }) {
  ...
  const normalizedBase = Math.max(0, Math.floor(Number(baseCoins) || BASE_VICTORY_COINS));
  const subtotal = normalizedBase + existingBonuses.fastVictory + existingBonuses.onePokemonVictory;
  const champion = championBonusEligible ? Math.round(subtotal * (BADGE_CHAMPION_COIN_MULTIPLIER - 1)) : 0;
  return { base: normalizedBase, bonuses: { ...existingBonuses, champion }, total: subtotal + champion };
}
