...
const debugTournament = (event, detail) => {
  if (isDevelopment) console.info(`[Tournament] ${event}`, detail);
};
const client = () => {
  if (!url || !key)
    throw new Error(
      "Configure as variáveis do Supabase para usar Campeonatos.",
    );
...
const normalizeCode = (value) =>
  String(value || "")
    ...
    .replace(/^PKC/, "")
    .slice(0, 4);
export const formatTournamentCode = (value) =>
  `${TOURNAMENT_CONFIG.codePrefix}-${normalizeCode(value)}`;
const code = () =>
  `${TOURNAMENT_CONFIG.codePrefix}-${Math.floor(1000 + Math.random() * 9000)}`;
const error = (message) => {
...
export async function createTournament(profile) {
  ...
  for (let tries = 0; tries < 4; tries += 1) {
    ...
    const { data: tournament, error: createError } = await db
      .from("tournaments")
      .insert({ code: tournamentCode, created_by_player_id: profile.playerId })
      .select()
      .single();
...
export async function joinTournament(rawCode, profile) {
  const db = client();
  const tournamentCode = formatTournamentCode(rawCode);
  const timestamp = new Date().toISOString();
  const { data: tournament, error: lookupError } = await db
    .from("tournaments")
    .select("*")
  ...
  if (tournament.status !== TOURNAMENT_STATUS.LOBBY)
  ...
  debugTournament("JOIN ATTEMPT", {
    tournamentId: tournament.id,
    tournamentCode,
    localPlayerId: profile.playerId,
    displayName: profile.displayName,
    currentParticipantCount: players.length,
    currentParticipantIds: players.map((item) => item.player_id),
    requestedSlot: "database-assigned",
    clientTimestamp: timestamp,
  });
  debugTournament("VALIDATION RESULT", {
    alreadyJoined: players.some((item) => item.player_id === profile.playerId),
    isFull: players.length >= TOURNAMENT_CONFIG.playerCount,
    tournamentStatus: tournament.status,
    allowed:
      tournament.status === TOURNAMENT_STATUS.LOBBY &&
      !players.some((item) => item.player_id === profile.playerId) &&
      players.length < TOURNAMENT_CONFIG.playerCount,
  });
  debugTournament("DATABASE REQUEST START", {
    operation: "join_tournament",
    tournamentId: tournament.id,
  });
  const { data: result, error: joinError } = await db.rpc("join_tournament", {
  ...
  if (joinError) {
    ...
    if (joinError.code === "PT002")
      return error("Este campeonato já está completo.");
    if (joinError.code === "PT003")
      return error("Este campeonato já foi iniciado, cancelado ou finalizado.");
    throw joinError;
  }
  ...
  if (result?.alreadyJoined) {
    ...
    debugTournament("JOIN REJECTED — ALREADY PARTICIPANT", {
      localPlayerId: profile.playerId,
      localDisplayName: profile.displayName,
      matchedDatabasePlayerId: matched?.player_id || null,
      matchedDisplayName: matched?.display_name || null,
      tournamentId: tournament.id,
    });
  }
...
export function subscribeTournament(tournamentId, onChange) {
  ...
  const channel = db
    ...
    .on(
      ...
      (payload) => {
        debugTournament("REALTIME EVENT", {
          eventType: payload.eventType,
          playerId: payload.new?.player_id || payload.old?.player_id,
          slot: payload.new?.slot || payload.old?.slot,
          tournamentId,
        });
        onChange(payload);
      },
...
export async function getTournament(idOrCode) {
  ...
  const query = db
    .from("tournaments")
    .select("*, tournament_players(*), tournament_matches(*)");
  const { data, error: readError } = String(idOrCode).startsWith("PKC-")
    ? await query.eq("code", idOrCode).maybeSingle()
...
export async function getPlayerActiveTournament(playerId) {
  ...
  const { data, error: readError } = await db
    ...
    .in("tournaments.status", [
      TOURNAMENT_STATUS.LOBBY,
      TOURNAMENT_STATUS.SEMIFINALS,
...
export async function startTournament(tournamentId, organizerId) {
  ...
  if (current.created_by_player_id !== organizerId)
    return error("Apenas quem criou pode iniciar o campeonato.");
  if (
    current.status !== TOURNAMENT_STATUS.LOBBY ||
    current.tournament_players.length !== TOURNAMENT_CONFIG.playerCount
  )
    return error("São necessários 4 jogadores para iniciar.");
  const { error: startError } = await db.rpc("start_tournament", {
...
export async function cancelTournament(tournamentId, organizerId) {
  ...
  if (!current) return error("Campeonato não encontrado.");
  if (current.created_by_player_id !== organizerId)
    return error("Apenas quem criou pode cancelar o campeonato.");
  if (
    ![
      TOURNAMENT_STATUS.LOBBY,
      TOURNAMENT_STATUS.SEMIFINALS,
    ...
    return error("Este campeonato não pode mais ser cancelado.");
  const { error: cancelError } = await db.rpc("cancel_tournament", {
    p_tournament_id: tournamentId,
    p_organizer_id: organizerId,
  });
...
export async function leaveTournament(tournamentId, playerId) {
  const db = client();
  const { error: leaveError } = await db.rpc("leave_tournament", {
    p_tournament_id: tournamentId,
...
export async function markTournamentMatchPlaying(matchId) {
  const db = client();
  const { error: markError } = await db.rpc("mark_tournament_match_playing", {
    p_match_id: matchId,
  });
...
export async function completeTournamentMatch(matchId, winnerId) {
  const db = client();
  const { data: match, error: readError } = await db
